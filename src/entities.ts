...
export function makePlayer(k: KaboomCtx, posX: number, posY: number) {
    ...
    const inhaleEffect = k.add([
        k.sprite("assets", {anim: "heroInhalingEffect"}),
        k.pos(),
        k.scale(scale),
        k.opacity(0),
        "inhaleEffect"
    ...
    player.onUpdate(() => { // When a player falls
...
export function setControls(k: KaboomCtx, player: PlayerGameObj) {
    ...
    k.onKeyDown((key) => {
        ...
        switch (key) {
            ...
            case "z":
                if (player.isFull) {
                    player.play("heroFull");
                    inhaleEffectRef.opacity = 0;
                    break;
                }
                
                player.isInhaling = true;
                player.play("heroInhaling");
    ...
    k.onKeyRelease((key) => {
        if (key === "z") {
            if (player.isFull) {
                ...
                player.isFull = false;
                k.wait(1, () => player.play("heroIdle"));
                return;
            }

            inhaleEffectRef.opacity = 0;
            player.isInhaling = false;
            player.play("heroIdle");
        }
    });
}
